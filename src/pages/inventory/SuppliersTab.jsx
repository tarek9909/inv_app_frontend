...
import { Plus, Edit, Archive, RotateCcw, Trash2, Search, AlertCircle, RefreshCw, Truck, Phone, Mail } from 'lucide-react';
...
export default function SuppliersTab() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (editing) { const { status, ...payload } = form; await inventoryStores.suppliers.update(editing.id, payload); toast.success('Supplier updated'); }
      else { await inventoryStores.suppliers.create(form); toast.success('Supplier created'); }
      setModalOpen(false);
  ...
  const toggleArchive = async (row) => {
    try { await inventoryStores.suppliers.setStatus(row.id, row.status === 'inactive' ? 'active' : 'inactive'); toast.success(row.status === 'inactive' ? 'Restored' : 'Archived'); inventoryStores.suppliers.load(); }
    catch (err) { toast.error(err?.message || 'Failed'); }
  };

  const confirmDelete = async () => {
    try { await inventoryStores.suppliers.delete(deleteTarget.id); toast.success('Supplier deleted'); } catch (err) { toast.error(err?.message || 'Delete failed'); }
    ...
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: '1', minWidth: '200px', maxWidth: '320px' }}>
          ...
          <input type="text" value={search} onChange={(e) => handleSearch(e.target.value)} placeholder="Search suppliers..." className="glass-input" style={{ paddingLeft: '36px', padding: '10px 12px 10px 36px', fontSize: '13px' }} />
        </div>
        <RefreshButton onClick={() => inventoryStores.suppliers.load({ force: true })} loading={loading} />
        <div style={{ marginLeft: 'auto' }}>{can('suppliers.manage') && <button className="glass-button" style={{ fontSize: '13px', padding: '10px 18px' }} onClick={openCreate}><Plus size={16} /> Add Supplier</button>}</div>
      </div>
              ...
              <motion.div key={sup.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: idx * 0.03, duration: 0.25 }} className="glass-card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: '148px' }}>
                ...
                <div style={{ padding: '12px 14px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    {can('suppliers.manage') && <CardAction icon={Edit} label="Edit" onClick={() => openEdit(sup)} color="var(--text-secondary)" />}
                    {can('suppliers.archive') && <CardAction icon={sup.status === 'active' ? Archive : RotateCcw} label={sup.status === 'active' ? 'Archive' : 'Restore'} onClick={() => toggleArchive(sup)} color="var(--accent-purple)" />}
                    {can('suppliers.delete') && <CardAction icon={Trash2} label="Delete" onClick={() => setDeleteTarget(sup)} color="var(--accent-red)" />}
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      {!loading && !error && filteredRows.length > 0 && <Pagination page={pagination.page} pages={pagination.pages} total={pagination.total} onPageChange={pagination.setPage} label="suppliers" />}

      <Modal open={modalOpen} title={editing ? 'Edit Supplier' : 'Add Supplier'} onClose={() => setModalOpen(false)} width="520px">
        <form onSubmit={handleSubmit}>
          <FormField label="Name" required error={errors.name}><FormInput value={form.name} onChange={(v) => { setForm({ ...form, name: v }); setErrors({}); }} placeholder="Supplier name" /></FormField>
          <FormField label="Phone"><FormInput value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} placeholder="Phone number" /></FormField>
          <FormField label="Email"><FormInput type="email" value={form.email} onChange={(v) => setForm({ ...form, email: v })} placeholder="Email" /></FormField>
          <FormField label="Address"><FormTextarea value={form.address} onChange={(v) => setForm({ ...form, address: v })} placeholder="Address" rows={2} /></FormField>
          <FormField label="Notes"><FormTextarea value={form.notes} onChange={(v) => setForm({ ...form, notes: v })} placeholder="Notes" rows={2} /></FormField>
      ...
      <ConfirmModal open={!!deleteTarget} title="Delete Supplier" message={`Delete ${deleteTarget?.name}? Suppliers linked to items or orders cannot be deleted.`} onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} />
    </>
  );
}
